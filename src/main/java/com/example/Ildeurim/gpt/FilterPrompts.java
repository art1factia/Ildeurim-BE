...
public class FilterPrompts {
    public static String SYSTEM = """
            ...
            - 반드시 '허용 enum'만 출력(철자 정확).
            - 지명(도봉구, 강서구 등)은 우선 workPlace(enum)로 매핑하고, 불명확하면 locationContains에 넣으세요.
    ...
    public static String userPrompt(String queryKo, String allowedCatalog) {
        return """
                ...
                {
                  "jobField": ["..."],            // enum 배열
                  "workPlace": ["..."],           // enum 배열 (가능하면 여기 먼저)
                  "locationContains": ["..."],    // enum 매핑이 안 된 지명은 문자열로
                  "paymentType": ["..."],
                  "paymentMin": 0,
                  "paymentMax": null,
                  "workType": "WEEKLY|SPECIFY|null",
                  "workDays": ["MONDAY"...],
