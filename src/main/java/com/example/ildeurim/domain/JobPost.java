...
public class JobPost {
    ...
    @Column(nullable = false)
    private JobPostEnum.PaymentType paymentType;       // 급여 유형
    ...
    @Enumerated(EnumType.STRING)
    @ElementCollection(targetClass = JobPostEnum.ApplyMethod.class)
    private List<JobPostEnum.ApplyMethod> applyMethods; // 지원 방법 (간편지원, 전화, 이메일 등)
    ...
    @Column(nullable = false)
    private JobPostEnum.JobPostStatus status;          // OPEN / CLOSED
